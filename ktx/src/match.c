...
// FIXME: TODO: need serve ghosted players too!!!
void PlayersStats ()
{
	gedict_t	*p, *p2;
	float		f1;
	char		*tmp, *tmp2;
	int			tp, first, needline;
	...
	tp = !atoi ( ezinfokey ( world, "k_duel" ) ) && f1 && teamplay;
		...
		if( !strnull ( p->s.v.netname ) && p->k_accepted == 2 && !p->ready ) {
			...
			while ( p2 ) {
				...
				if( streq ( tmp, tmp2 ) ) {
					...
					needline = find ( p2, FOFCLSN, "player" ) && find ( p, FOFCLSN, "player" );

					G_bprint(2, "\x87 %s:\n"
								"  %d (%d) %s%.1f%%\n"
								"%s", p2->s.v.netname,
								(int)p2->s.v.frags,	(int)(p2->s.v.frags - p2->deaths),
								( tp ? va("%d ", (int)p2->friendly ) : "" ),
								p2->efficiency,
								( needline ? "ùûûûûûûûûü\n" : "" ) );
...
//				self->cnt = 0;
				...
				if( f4 == 2 && f4 < 3)
				{
					...
					if( player1scores == player2scores )
					{
						...
						if( k_overtime == 1 ) {
							...
							G_bprint(2, "ê%dë minute%s overtime follows\n", 
									(int)k_exttime, ((k_exttime != 1) ? "'s" : ""));
							self->s.v.nextthink = g_globalvars.time + 1;

							return;	
						} else {
							G_bprint(2, "Sudden death ÔˆÂÚÙÈÌÂ ‚ÂÁÈÓÛ\n");
		...
		G_bprint(2, "ê%dë minute%s remaining\n", (int)self->cnt, ((self->cnt != 1) ? "'s" : ""));
		...
		if( k_showscores ) {
			if( k_scores1 == k_scores2 ) {
				...
				G_bprint(2, "‘Â·Ì ê%së leads by %d frag%s\n",
...
// Spawns the timer and starts the countdown.
{
	...
	if( f1 == 2 )
		G_bprint(2, "ÀÔÌ‚·Ù ‘Â·ÌÛ ÚÂÛ·˜ÓÛ\n"); // komabt teams respawns
	else if( f1 == 1 )
		G_bprint(2, "À‘ ”·˜Ó”·ÊÂÙ˘\n");	// kt spawnsafety
	else
		G_bprint(2, "ŒÔÚÌ·Ï —◊ ÚÂÛ·˜ÓÛ\n"); // normal
...
// print qizmo ( FPD ) settings
	...
	if( iFpd & 170 ) {
		G_bprint(2, "QiZmo:");
		if( iFpd & 2 )
			G_bprint(2, " ÙÈÌÂÚ"); // timer
		if( iFpd & 8 )
			G_bprint(2, " Ï·Á");   // lag
		if( iFpd & 32 )
			G_bprint(2, " ÂÓÂÌ˘"); // enemy
		if( iFpd & 128 )
			G_bprint(2, " ÔÈÓÙ"); // point
...
// Called by a player to inform that (s)he is ready for a match.
{
	...
	if( CheckMembers( f1 ) ) {
		...
		G_bprint(2, "”ÂÚˆÂÚ ˜·ÓÙÛ ·Ù ÏÂ·ÛÙ %d Ï·˘ÂÚÛ ÈÓ Â·„Ë ÙÂ·Ì\nWaiting...\n", 
...
void PlayerBreak ()
{
	...
	if( self->k_vote ) {
		G_bprint(2, "%s ˜ÈÙË‰Ú·˜Û ", self->s.v.netname);
