...
void Deathmsg ()
{
    ...
    G_bprint(2, "Ãõóôïí äåáôèíåóóáçåó ");
...
void KickThink ()
{
    G_sprint( PROG_TO_EDICT( self->s.v.owner ) , 2, "Your ëéãë mode has timed out.\n");
...
void AdminKick ()
{
    ...
    if( self->k_captain )
    {
        G_sprint(self, 2, "Exit ãáðôáéî mode first\n");
        return;
    }
    ...
    else
    {
        ...
        G_sprint(self, 2, "Kicking process started\n"
						  ...
						  "Type ù to kick, î for next, ëéãë to leave.\n");
...
void ReqAdmin ()
{
    ...
    if( self->k_admin == 1.5 )
    {
        G_sprint(self, 2, "Abort åìåãôion first\n");
        return;
    }
...
void TogglePreWar ()
{
    ...
    if( tmp )
    {
        ...
        if( !match_in_progress )
        {
            G_bprint(2, "Players may îïô fire before match!\n");
            ...
            G_sprint(self, 2, "Players may îïô fire before match!\n");
...
void ToggleMaster ()
{
    ...
    if( f1 )
        G_bprint(2, "%s sets mastermode!\nPlayers may îïô alter settings\n", self->s.v.netname);
    else
