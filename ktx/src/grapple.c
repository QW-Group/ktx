...
void GrappleService()
{
  ...
  if ( vlen(hookVector) <= 100 ) // cancel chain sound
  {
    ...
    if ( self->lefty )
    {
     ...
     self->lefty = false; // reset the sound channel.
   }
 }
}
...
// fixme: use new variable instead of regen_time
...
void FreezeGravity( gedict_t *p )
{
  ...
  {
    if ( p->regen_time )              
      p->regen_time = g_globalvars.time + 0.5;
    else if ( g_globalvars.time > p->regen_time ) 
    {
      if ( p->gravity != 0 )
      {
	p->gravity = 0;
