...
static void json_weap_detail(fileHandle_t handle, const char* name, int weapon_num, wpType_t* stats)
{
	...
	if (stats->attacks || stats->hits || stats->rhits || stats->vhits) {
		s2di(handle, INDENT10 "\"acc\": { \"attacks\": %d, \"hits\": %d", stats->attacks, stats->hits);
	...
	if (stats->kills || stats->tkills || stats->ekills || stats->suicides) {
		...
		s2di(handle, INDENT10 "\"kills\": { \"total\": %d, \"team\": %d, \"enemy\": %d, \"self\": %d }", stats->kills, stats->tkills, stats->ekills, stats->suicides);
	}
	if (deathmatch < 4) {
		COMMA_CHECK(handle, any);
		s2di(handle, INDENT10 "\"deaths\": %d", stats->deaths);
	}
	if (deathmatch < 4 && (stats->drops || stats->tooks || stats->ttooks || stats->stooks || stats->sttooks)) {
	...
	if (stats->edamage || stats->tdamage) {
		COMMA_CHECK(handle, any);
		s2di(handle, INDENT10 "\"damage\": { \"enemy\": %d, \"team\": %d }", stats->edamage, stats->tdamage);
	}
...
void json_player_detail(fileHandle_t handle, int player_num, gedict_t* player, const char* team)
{
	...
	s2di(handle, INDENT6 "\"dmg\": { \"taken\": %d, \"given\": %d, \"team\": %d, \"self\": %d, \"team-weapons\": %d, \"enemy-weapons\": %d, \"taken-to-die\": %d }," JSON_CR,
		...
		(int)player->ps.dmg_tweapon, (int)player->ps.dmg_eweapon, (int)player->deaths == 0 ? 99999 : (int)player->ps.dmg_t / (int)player->deaths
	);
	s2di(handle, INDENT6 "\"xfer\": %d," JSON_CR, player->ps.transferred_packs);
	s2di(handle, INDENT6 "\"spree\": { \"max\": %d, \"quad\": %d }," JSON_CR, player->ps.spree_max, player->ps.spree_max_q);
	s2di(handle, INDENT6 "\"control\": %f," JSON_CR, player->ps.control_time);
	s2di(handle, INDENT6 "\"speed\": { \"max\": %f, \"avg\": %f }," JSON_CR, player->ps.velocity_max, player->ps.vel_frames > 0 ? player->ps.velocity_sum / player->ps.vel_frames : 0.);
...
void json_match_header(fileHandle_t handle, char* ip, int port)
{
	...
	s2di(handle, INDENT2 "\"hostname\": \"%s\"," JSON_CR, json_string(striphigh(cvar_string("hostname"))));
...
static void json_player_instagib_stats(fileHandle_t handle, player_stats_t* stats)
{
	...
	COMMA_CHECK(handle, any);
	s2di(handle, INDENT8 "\"gibs\": { \"coil\": %d, \"axe\": %d, \"stomp\": %d, \"multi\": %d, \"air\": %d, \"best-multi\": %d }" JSON_CR, stats->i_cggibs, stats->i_axegibs, stats->i_stompgibs, stats->i_multigibs, stats->i_airgibs, stats->i_maxmultigibs);
	s2di(handle, INDENT6 "}");
}
...
static void json_player_midair_stats(fileHandle_t handle, player_stats_t* stats)
{
	...
	if (stats->mid_bronze || stats->mid_silver || stats->mid_gold || stats->mid_platinum) {
		COMMA_CHECK(handle, any);
		s2di(handle, INDENT8 "\"midairs\": { \"bronze\": %d, \"silver\": %d, \"gold\": %d, \"platinum\": %d }", stats->mid_bronze, stats->mid_silver, stats->mid_gold, stats->mid_platinum);
	}
	if (stats->mid_total) {
		COMMA_CHECK(handle, any);
	...
	if (stats->mid_totalheight || stats->mid_maxheight || stats->mid_avgheight) {
		COMMA_CHECK(handle, any);
		s2di(handle, INDENT8 "\"heights\": { \"total\": %f, \"max\": %f, \"avg\": %f }", stats->mid_totalheight, stats->mid_maxheight, stats->mid_avgheight);
	}
	NEWLINE_CHECK(handle, any);
	s2di(handle, INDENT6 "}");
}
...
void json_race_detail(fileHandle_t handle)
{
	...
	s2di(handle, INDENT4 "\"weapon-mode\": \"%s\"," JSON_CR, race.weapon == raceWeaponAllowed ? "allowed" : race.weapon == raceWeapon2s ? "delayed" : "none");
	s2di(handle, INDENT4 "\"can-false-start\": %s," JSON_CR, race.falsestart == raceFalseStartYes ? "true" : "false");
	s2di(handle, INDENT4 "\"match\": %s," JSON_CR, race_match_mode() ? "true" : "false");
	if (!strnull(race.pacemaker_nick)) {
		s2di(handle, INDENT4 "\"pacemaker\": { \"time\": %.3f, \"name\": \"%s\" }," JSON_CR, race.pacemaker_time * 0.001f, json_string(race.pacemaker_nick));
	}
	if (race_match_mode()) {
		int player_count;
		int i;
		...
		for (i = 0; i < player_count; ++i) {
			...
			s2di(handle, INDENT6 "{ \"bestTime\": %.3f, \"completions\": %d, \"score\": %d, "
				...
				json_string(stats[i].name), stats[i].total_distance, stats[i].total_time / 1000.0f, stats[i].wins
			);
		}
		NEWLINE_CHECK(handle, any);
		s2di(handle, INDENT4 "]" JSON_CR);
