...
static vec3_t straight_up =
	{ 0, 0, 1 };
static vec3_t straight_down =
	{ 0, 0, -1 };
...
{
	traceline(self->s.v.origin[0], self->s.v.origin[1], self->s.v.origin[2], self->s.v.origin[0],
				self->s.v.origin[1], self->s.v.origin[2] + 140, true, self);
...
void BotCanRocketJump(gedict_t *self)
{
	...
	else
	{
		...
		float health_after = TotalStrengthAfterDamage(self->s.v.health, self->s.v.armorvalue,
														self->s.v.armortype,
														tp_damage ? 55 * (has_quad ? 4 : 1) : 0);
...
{
	if (isCTF() && cvar("k_ctf_hook") > 0)
	{
		self->fb.canHook = true;
	}
	else
	{
		self->fb.canHook = false;
	}
}
		...
		switch (self->fb.lavaJumpState)
		{
			case FB_LAVAJUMP_NOT:
				if ((self->s.v.waterlevel >= 2) && BotCheckSpaceAbove(self))
				{
					...
					VectorSubtract(self->s.v.origin, self->fb.linked_marker->s.v.origin,
									linked_direction);
			...
			case FB_LAVAJUMP_RISE:
				...
				else
				{
					...
					FindRocketExplosionPoint(self->s.v.origin, self->fb.desired_angle,
												explosion_point, &distance_frac);
					BotLavaJumpRise(self);

					frames_to_explosion = (VectorDistance(self->s.v.origin, explosion_point) / 1000)
...
void BotPerformRocketJump(gedict_t *self)
{
	...
	else if (self->fb.canRocketJump && ((int)self->s.v.flags & FL_ONGROUND))
	{
		...
		qbool ok_direction = DirectionCheck(self->fb.touch_marker->s.v.origin,
											self->fb.linked_marker->s.v.origin, self->s.v.velocity,
											0.8f); // FIXME: TODO
...
void BotPerformHook(gedict_t *self)
{
	...
	else if (self->fb.canHook)
	{
		...
		if (path_is_hook && ok_distance && ok_target)
		{
