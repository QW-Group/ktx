						...
						float current_goal_time, float current_goal_time_125)
{
	...
	if ((eval->description & HOOK) && allowHook)
	{
		...
		// The real pull speed varies with k_ctf_hookstyle, the haste rune and the
		// player's launch velocity, none of which are known when route costs are
		// precomputed, so approximate using the nominal steady-state pull speed.
		eval->path_time = (VectorDistance(marker_position, m_pos) / PULL_SPEED);
	}
