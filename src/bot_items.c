...
static float goal_defend_marker(gedict_t *self, gedict_t *marker)
{
	...
	if (marker->fb.T & MARKER_FLAG1_DEFEND && streq(getteam(self), "blue"))
	{
		return 0;
	}
	if (marker->fb.T & MARKER_FLAG2_DEFEND && streq(getteam(self), "red"))
	{
		return 0;
	}

	if (VectorDistance(self->s.v.origin, marker->s.v.origin) < 300)
	{
		return 0; // BAD!
	}
