...
static vec3_t first_trace_plane_normal =
	{ 0 };
...
static float StandardFallHeight(gedict_t *self)
{
	...
	{
		float min_second = self->fb.linked_marker->s.v.absmin[2]
				+ self->fb.linked_marker->s.v.view_ofs[2] - 36; // FIXME: Why 36?
...
static void TestTopBlock(gedict_t *self, vec3_t last_clear_point, vec3_t testplace)
{
	...
	for (i = 0; i < 4; ++i)
	{
		traceline(last_clear_point[0] + xDelta[i], last_clear_point[1] + yDelta[i],
					last_clear_point[2] + 32, testplace[0] + xDelta[i], testplace[1] + yDelta[i],
					testplace[2] + 32, true, self);
...
static void TestBottomBlock(gedict_t *self, vec3_t last_clear_point, vec3_t testplace)
{
	...
	{
		traceline(last_clear_point[0] + xDelta[i], last_clear_point[1] + yDelta[i],
					last_clear_point[2] - 24, testplace[0] + xDelta[i], testplace[1] + yDelta[i],
					testplace[2] - 24, true, self);
		if (g_globalvars.trace_fraction != 1)
		{
			if (g_globalvars.trace_plane_normal[2] >= 0)
			{
...
static void HazardTeleport(gedict_t *teleport, gedict_t *teleported_player)
{
	...
	teleport->fb.arrow_time = max(teleport->fb.arrow_time,
									g_globalvars.time + ARROW_TIME_AFTER_TELEPORT);
...
static void ExplodeAlert(vec3_t org, float next_time)
{
	...
	for (marker = world; (marker = trap_findradius(marker, org, 256));)
	{
		if (marker->fb.fl_marker)
		{
			...
			traceline(org[0], org[1], org[2], marker->s.v.absmin[0] + marker->s.v.view_ofs[0],
						marker->s.v.absmin[1] + marker->s.v.view_ofs[1],
						marker->s.v.absmin[2] + marker->s.v.view_ofs[2], true, marker);
			if (g_globalvars.trace_fraction == 1)
			{
...
static void RocketAlert(void)
{
	...
	traceline(PASSVEC3(self->s.v.origin), PASSVEC3(end_point), true,
				PROG_TO_EDICT(self->s.v.owner));
...
static int FallSpotGround(vec3_t testplace, float fallheight)
{
	...
	if (walkmove(dropper, 0, 0))
	{
		...
		content = trap_pointcontents(dropper->s.v.origin[0], dropper->s.v.origin[1],
										dropper->s.v.origin[2] - 24);
...
static qbool CanJumpOver(gedict_t *self, vec3_t jump_origin, vec3_t jump_velocity, float fallheight,
							int current_fallspot)
{
	int tries = 0;
	...
	for (tries = 0; tries < 20; ++tries)
	{
		...
		if (fall == FALL_BLOCKED)
		{
			...
			if (first_trace_fraction != 1)
			{
				...
				VectorMA(last_clear_point, (first_trace_fraction * 32 / last_clear_hor_speed),
							last_clear_velocity, testplace);

				dotProduct = DotProduct(first_trace_plane_normal, last_clear_velocity);
				VectorMA(last_clear_velocity, -dotProduct, first_trace_plane_normal,
							last_clear_velocity);
				...
				VectorMA(testplace, (32 / last_clear_hor_speed) * (1 - first_trace_fraction),
							last_clear_velocity, testplace);
			}
	...
	if (being_blocked && g_globalvars.time > self->fb.arrow_time)
	{
		if (self->fb.ledge_backup_time)
		{
			if (g_globalvars.time >= self->fb.ledge_backup_time)
			{
				...
				VectorMA(rel_hor_dir, -DotProduct(self->fb.obstruction_normal, rel_hor_dir),
							self->fb.obstruction_normal, dir_move);
...
static void ObstructionLogic(gedict_t *self, vec3_t new_velocity)
{
	...
	if (self->fb.linked_marker != self->fb.touch_marker)
	{
		if (vlen(self->fb.oldvelocity) <= 32)
		{
			...
			VectorMA(new_velocity, -DotProduct(self->fb.obstruction_normal, new_velocity),
						self->fb.obstruction_normal, dir_move);
			...
			{
				VectorMA(self->fb.dir_move_,
							-0.5 * DotProduct(self->fb.obstruction_normal, self->fb.dir_move_),
							self->fb.obstruction_normal, dir_move);
			}
	...
	VectorMA(new_velocity, -DotProduct(self->fb.obstruction_normal, new_velocity),
				self->fb.obstruction_normal, new_velocity);
}
...
static void AvoidHazardsOnGround(gedict_t *self, float hor_speed, vec3_t new_origin,
									vec3_t new_velocity, vec3_t dir_forward)
{
	int fall = 0;
	int new_fall = 0;
	vec3_t jump_origin =
	...
	snprintf(debug[line++], sizeof(debug[0]),
				"AvoidHazardsOnGround(origin [%d %d %d], vel [%d %d %d])\n",
	...
	if (fall == FALL_BLOCKED)
	{
		...
		if (first_trace_fraction != 1)
		{
			...
			VectorMA(last_clear_point, (16 / hor_speed) * first_trace_fraction, new_velocity,
						testplace);
			VectorMA(new_velocity, -DotProduct(first_trace_plane_normal, new_velocity),
						first_trace_plane_normal, new_velocity);
			VectorCopy(new_velocity, hor_velocity);
			hor_velocity[2] = 0;
			hor_speed = vlen(hor_velocity);
			VectorMA(testplace, (16 / hor_speed) * (1 - first_trace_fraction), new_velocity,
						testplace);
		}
	...
	if (fall >= FALL_LAND)
	{
		...
		if (new_fall > fall)
		{
			...
			traceline(self->s.v.origin[0] + (dir_forward[0] * 32),
			...
			if ((g_globalvars.trace_plane_normal[0] == 0) && (g_globalvars.trace_plane_normal[1] == 0)
					&& (g_globalvars.trace_plane_normal[2] == 0))
			{
				AvoidEdge(self);
...
static void AvoidHazardsInAir(gedict_t *self, float hor_speed, vec3_t new_origin,
								vec3_t new_velocity, vec3_t last_clear_point)
{
	float fallheight = StandardFallHeight(self);
	vec3_t testplace;
	int fall;
...
void AvoidHazards(gedict_t *self)
{
	...
	else if (self->fb.obstruction_normal[0] || self->fb.obstruction_normal[1]
