...
static antilag_client_info_t antilag_info[MAX_CLIENTS][MAX_CLIENTS];

void Antilag_Init(void)
{
	memset( antilag_info, 0, sizeof(antilag_info) );
}

void Antilag_Reset(	gedict_t *s )
{
	int i;
	int player = NUM_FOR_EDICT( s ) - 1;
	...
	{
		antilag_info[player][i].active = false;
		antilag_info[player][i].applied = false;
	}
}
...
void Antilag_Apply(	gedict_t *s )
{
	...
	for ( i = 0; i < MAX_CLIENTS; i++ )
	{
		if ( !antilag_info[player][i].active || antilag_info[player][i].applied )
			continue;
		
		e = g_edicts + i + 1;

		if ( e->ct != ctPlayer || ISDEAD( e ) )
			continue;
		...
		if ( player == i )
		{
			...
			VectorSubtract(e->s.v.origin, antilag_info[player][i].origin, v);
			G_sprint( s, 2, "self offset %d\n", (int)vlen(v));
		}
#endif

		// save origin
		VectorCopy( e->s.v.origin, antilag_info[player][i].apply_origin );
		// move to the new origin
		setorigin( e, PASSVEC3( antilag_info[player][i].origin ) );

		antilag_info[player][i].applied = true;

#if 0
		G_sprint( s, 2, "apply %s %d %d %d\n", getname(e),
							(int)antilag_info[player][i].origin[0],
							(int)antilag_info[player][i].origin[1],
							(int)antilag_info[player][i].origin[2]);
#endif
	}
}
...
void Antilag_Undo(	gedict_t *s )
{
	...
	for ( i = 0; i < MAX_CLIENTS; i++ )
	{
		if ( !antilag_info[player][i].applied )
			continue;
		
		e = g_edicts + i + 1;

		if ( e->ct != ctPlayer || ISDEAD( e ) )
			continue; // FIXME: I am not sure about ISDEAD(), probably its better remove it

		// restore origin
		setorigin( e, PASSVEC3( antilag_info[player][i].apply_origin ) );
...
{
	char	arg[64];
	int		idx, argc, other_num, self_num;

	if ( self->ct != ctPlayer )
		return; // something wrong

	self_num = NUM_FOR_EDICT( self ) - 1;
	if ( self_num < 0 || self_num >= MAX_CLIENTS )
		G_Error("cmd_al: self_num < 0 || self_num >= MAX_CLIENTS");

	argc = trap_CmdArgc();

	for ( idx  = 0; idx + 4 < argc; idx += 4 )
	{
		// player num
		trap_CmdArgv( idx + 1, arg, sizeof( arg ) );
		other_num = atoi(arg);

		if (   other_num < 0 || other_num >= MAX_CLIENTS
			/* || self_num == other_num */ || g_edicts[other_num + 1].ct != ctPlayer
			)
			continue; // illegitimate

		// make it active
		antilag_info[self_num][other_num].active = true;

		// x
		trap_CmdArgv( idx + 2, arg, sizeof( arg ) );
		antilag_info[self_num][other_num].origin[0] = atoi(arg);
		// y
		trap_CmdArgv( idx + 3, arg, sizeof( arg ) );
		antilag_info[self_num][other_num].origin[1] = atoi(arg);
		// z
		trap_CmdArgv( idx + 4, arg, sizeof( arg ) );
		antilag_info[self_num][other_num].origin[2] = atoi(arg);
	}
}
