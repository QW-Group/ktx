...
static gedict_t* TeamplayFindPoint(gedict_t *client)
{
	...
	for (i = 0; i < MAX_EDICTS; i++)
	{
		...
		visitem.radius =
				(int)e->s.v.effects & (EF_BLUE | EF_RED | EF_DIMLIGHT | EF_BRIGHTLIGHT) ?
						200 : max(max(size[0] / 2, size[1] / 2), size[2] / 2);
...
static char* CTFItemText(gedict_t *client)
{
	...
	if (HAVE_FLAG(client))
	{
		if (streq(getteam(client), "red"))
		{
			strlcat(buffer, "{&c57fFLAG&cfff}", sizeof(buffer));
		}
		if (streq(getteam(client), "blue"))
		{
			strlcat(buffer, "{&cf00FLAG&cfff}", sizeof(buffer));
		}
		strlcat(buffer, " ", sizeof(buffer));
	}
	if (HAVE_RUNE(client))
	{
		if (client->ctf_flag & CTF_RUNE_RES)
		{
			strlcat(buffer, TP_NAME_RUNE1, sizeof(buffer));
		}
		if (client->ctf_flag & CTF_RUNE_STR)
		{
			strlcat(buffer, TP_NAME_RUNE2, sizeof(buffer));
		}
		if (client->ctf_flag & CTF_RUNE_HST)
		{
			strlcat(buffer, TP_NAME_RUNE3, sizeof(buffer));
		}
		if (client->ctf_flag & CTF_RUNE_RGN)
		{
			strlcat(buffer, TP_NAME_RUNE4, sizeof(buffer));
		}
	}
...
static void TeamplayReportNeeds(gedict_t *client)
{
	...
	buffer[0] = '\0';
	if (NEED(needFlags, it_health) || NEED(needFlags, it_armor) || NEED(needFlags, it_rl)
			|| NEED(needFlags, it_lg) || NEED(needFlags, it_rockets) || NEED(needFlags, it_cells)
...
static void TeamplayEnemyPowerup(gedict_t *client)
{
	...
	else if (point && point->ct == ctPlayer && !SameTeam(client, point))
	{
		...
		{
			strlcat(buffer, TeamplayLastEnemyPowerupText(client, &suppressLocation),
					sizeof(buffer));
		}
	}
	else if (HAVE_POWERUP(client))
	{
		...
		needFlags = TeamplayNeedFlags(client);
		if (NEED(needFlags, it_health) || NEED(needFlags, it_armor) || NEED(needFlags, it_rockets)
				|| NEED(needFlags, it_cells) || NEED(needFlags, it_rl) || NEED(needFlags, it_lg))
		{
...
static void TeamplayPoint(gedict_t *client)
{
	...
	if (point->ct == ctPlayer)
	{
		...
		{
			strlcat(buffer,
					va("{%d} %s at \20{%s}\21", client->tp.enemy_count, TP_NAME_ENEMY,
						LocationName(PASSVEC3(point->s.v.origin))),
					sizeof(buffer));
		}
		else
		{
	...
	else
	{
		...
		for (i = 0; i < (sizeof(item_flags) / sizeof(item_flags[0])); ++i)
		{
			...
			{
				strlcat(buffer,
						va("%s at \20{%s}\21 {%d}", item_names[i],
							LocationName(PASSVEC3(point->s.v.origin)), client->tp.enemy_count),
						sizeof(buffer));
				break;
			}
		}
