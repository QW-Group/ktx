...
stand1, stand2, stand3, stand4, stand5, stand6, stand7, stand8, stand9, stand10, stand11, stand12, stand13,

walk1, walk2, walk3, walk4, walk5, walk6, walk7, walk8,

run1, run2, run3, run4, run5, run6,

leap1, leap2, leap3, leap4, leap5, leap6, leap7, leap8, leap9, leap10, leap11, leap12,
...
ANIM(demon1_jump1,  leap1,  demon1_jump2;  ai_face(); sound( self, CHAN_VOICE, "demon/djump.wav", 1, ATTN_NORM );)
...
ANIM(demon1_jump10, leap10, demon1_jump1 /* NOTE it demon1_jump1 not demon1_jump11 */; _demon1_jump10();)
...
float CheckDemonJump()
{
	...
	if ( self->s.v.origin[2] + self->s.v.mins[2] > 
		 ...
		 PROG_TO_EDICT( self->s.v.enemy )->s.v.mins[2] + 0.75 * PROG_TO_EDICT( self->s.v.enemy )->s.v.size[2]
	   )
		return false;

	if ( self->s.v.origin[2] + self->s.v.maxs[2] <
		 PROG_TO_EDICT( self->s.v.enemy )->s.v.origin[2] + 
		 PROG_TO_EDICT( self->s.v.enemy )->s.v.mins[2] + 0.25 * PROG_TO_EDICT( self->s.v.enemy )->s.v.size[2]
	   )
		return false;
