...
void ProcessNewLinkedMarker(gedict_t* self) {
		...
		if (have_rl) {
			if (self->fb.firepower < enemy_->fb.firepower && self->s.v.armorvalue < enemy_->s.v.armorvalue) {
				...
				if (!self->fb.avoiding && enemy_->fb.touch_marker) {
					gedict_t* look_marker = HigherSightFromFunction (self->fb.touch_marker, enemy_->fb.touch_marker);
					if (!look_marker) {
						look_marker = SightMarker(self->fb.touch_marker, enemy_->fb.touch_marker, 1000.0f, 40.0f);
					}
					if (look_marker) {
	...
	PathScoringLogic (
		self->fb.goal_respawn_time, self->fb.be_quiet, self->fb.skill.lookahead_time, self->fb.path_normal_, 
		self->s.v.origin, player_direction, self->fb.touch_marker, goalentity_marker, rocket_alert, rocket_jump_routes_allowed, trace_bprint, 
		self,
		&best_score, &new_linked_marker, &new_path_state, &new_angle_hint, &new_rj_delay, new_rj_angles
	);
