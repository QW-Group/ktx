...
static qbool BotGoUpForAir(gedict_t *self, vec3_t dir_move)
{
	...
	if (g_globalvars.time > (self->air_finished - BOT_DROWN_SAFETY_TIME))
	{
		...
		traceline(self->s.v.origin[0], self->s.v.origin[1], self->s.v.origin[2],
					self->s.v.origin[0], self->s.v.origin[1], self->s.v.origin[2] + 64, true, self);
		...
		{
			traceline(self->s.v.origin[0], self->s.v.origin[1], self->s.v.origin[2],
						self->s.v.origin[0], self->s.v.origin[1], self->s.v.origin[2] + 32, true,
						self);
			if (g_globalvars.trace_fraction != 1)
			{
				return (self->fb.swim_arrow = UP);
...
static void SwimAwayFromWall(gedict_t *self, vec3_t dir_move)
{
	...
	traceline(self->s.v.origin[0], self->s.v.origin[1], self->s.v.origin[2],
				self->s.v.origin[0] + g_globalvars.v_right[0] * 20,
				self->s.v.origin[1] + g_globalvars.v_right[1] * 20,
				self->s.v.origin[2] + g_globalvars.v_right[2] * 20, true, self);
	if (g_globalvars.trace_fraction != 1)
	{
		vec3_t temp;
...
void BotWaterMove(gedict_t *self)
{
	...
	if (self->fb.obstruction_normal[0] || self->fb.obstruction_normal[1]
	...
	{
		traceline(self->s.v.origin[0], self->s.v.origin[1], self->s.v.origin[2],
					self->s.v.origin[0], self->s.v.origin[1], self->s.v.origin[2] + 32, true, self);
		if (g_globalvars.trace_fraction == 1)
		{
			self->fb.swim_arrow = UP;
		}
